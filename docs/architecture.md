...
The platform acts as an operational governance and optimization intelligence layer for AKS environments. It consumes Azure APIs, Kubernetes APIs, Azure Managed Prometheus, and Azure Monitor signals, then produces deterministic, explainable governance findings and resource recommendations.
...
The frontend and backend use separate app registrations. The frontend app signs in users. The backend app exposes the `access_as_user` scope and is the token audience. Azure Resource Graph, AKS, and Azure Monitor access is performed by the backend using a service-side Azure credential, which is `DefaultAzureCredential` locally and managed identity in production.
